...
export default function Page() {
  ...
  const generateOptimalSchedule = () => {
    ...
    // 제출된 학생들을 지망 개수가 적거나 선점이 필요한 순으로 정렬 후 매칭
    const studentsPool = [...allSubmissions];

    // 1순위부터 6순위까지 차례대로 최적의 타임슬롯 매칭
    studentsPool.forEach((student) => {
      ...
      if (student.ranks && Array.isArray(student.ranks)) {
        for (let r = 0; r < student.ranks.length; r++) {
          ...
          // 옵션으로 아직 유효하고(어드민 삭제 X), 다른 사람에게 아직 안 겹친 슬롯인 경우
      ...
      <header className="border-b border-slate-800 bg-slate-900/80 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-5xl mx-auto px-5 py-4 flex justify-between items-center">
          ...
          >
            ...
            <div>
              <h1 className="font-bold text-base text-slate-100 tracking-tight">면접 일정 지망 및 자동 배정 시스템</h1>
